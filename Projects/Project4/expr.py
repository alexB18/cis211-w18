...
class Env(object):
    ...
    def eval(self, var):
        ...
        if var in self.currently_evaluating:
            log.warning("Cyclic reference to {}?  Bailing.".format(var))
            return var
...
class BinOp(Expr):
    """ Abstract superclass for binary expressions like addition, subtraction, etc"""
    ...
    def eval(self):
        ...
        if isinstance(lval, Const) and isinstance(rval, Const):
            ...
            log.debug("Constructing new {}({},{})"
                      .format(type(self), lval, rval))
            return self

    def __eq__(self, other):
        """ check if type matches and both left/right nodes match"""
...
class Plus(BinOp):
    def _apply(self, left, right):
        ...
        assert isinstance(left, Const) and isinstance(right, Const)
        return Const(left.value() + right.value())

    def __repr__(self):
        return "Plus({},{})".format(repr(self.left),
                                    repr(self.right))
...
class Minus(BinOp):
    ...
    def __repr__(self):
        return "Minus({},{})".format(repr(self.left),
                              repr(self.right))
...
class Times(BinOp):
    ...
    def __repr__(self):
        return "Times({},{})".format(repr(self.left),
                                    repr(self.right))
...
class Div(BinOp):
    ...
    def __repr__(self):
        return "Div({},{})".format(repr(self.left),
                                   repr(self.right))
