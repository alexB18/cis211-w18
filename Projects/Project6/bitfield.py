...
class BitField(object):
    ...
    def insert(self, field_value: int, word: int) -> int:
        ...
        """
        masked = self.low_mask & field_value
        masked = masked << self.from_bit

        # Insert value into word
        # Ie: If there is a 1 already included, the | operation will simply leave it
        new_word = masked | word
    ...
    def extract(self, word: int) -> int:
        ...
        """
        masked = self.low_mask << self.from_bit

        # Extract word from field
        new_word = masked & word
        new_word = new_word >> self.from_bit
