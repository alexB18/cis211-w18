...
class Group(object):
    ...
    def hidden_single_constrain(self) -> bool:
        ...
        for val in sdk_tile.CHOICES:
            ...
            for tile in self.tiles:
                if tile.value == val:
                    # It already has a place; no need to
                    # look further for one
                    break
                if tile.could_be(val):
                    if placement is None:
                        # The first place we've found that it
                        # can go.  
                        placement = tile
                    else:
                        # There is at least one other place
                        # this choice could go. Hidden single
                        # won't help us place this tile.
                        break
            else:
                # Ran through the loop, didn't find the value
                # and didn't find two places to put it.  If
                # I found any at all, that's where it goes. 
                if placement:
                    placement.set_value(val)
                    changed = True
                    # Otherwise it is inconsistent.  Maybe raise
                    # an exception here, or otherwise announce
                    # inconsistency? 
        self.unattend()
        return changed
